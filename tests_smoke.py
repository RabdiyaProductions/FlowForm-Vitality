...
def test_library_equipment_filter_matches_template_tags(tmp_path, monkeypatch):
    monkeypatch.setenv('DB_PATH', str(tmp_path / 'library-equipment.db'))
    app = create_app(port=5450)
    client = app.test_client()

    page = client.get('/library?equipment=dumbbells')
    assert page.status_code == 200
    assert b'dumbbells' in page.data.lower()


def test_session_intensity_query_affects_computed_seconds(tmp_path, monkeypatch):
    import sqlite3, re, json
    monkeypatch.setenv('DB_PATH', str(tmp_path / 'intensity.db'))
    app = create_app(port=5451)
    client = app.test_client()
    ...
    low = client.get(f'/session/start/{plan_day_id}?intensity=low')
    high = client.get(f'/session/start/{plan_day_id}?intensity=high')
    assert low.status_code == 200 and high.status_code == 200

    m_low = re.search(rb'const sessionData = (\{.*?\});', low.data, re.S)
    m_high = re.search(rb'const sessionData = (\{.*?\});', high.data, re.S)
    assert m_low and m_high
    low_json = json.loads(m_low.group(1).decode('utf-8'))
...
def test_nav_links_are_unique_and_load(tmp_path, monkeypatch):
    import re
    monkeypatch.setenv('DB_PATH', str(tmp_path / 'nav-audit.db'))
    app = create_app(port=5453)
    client = app.test_client()

    page = client.get('/dashboard')
    assert page.status_code == 200
    nav_block = re.search(rb'<nav class="nav">(.*?)</nav>', page.data, re.S)
    assert nav_block
    links = re.findall(rb'<a href="([^"]+)"', nav_block.group(1))
    nav_links = [l.decode('utf-8') for l in links if l.startswith(b'/')]
    # keep primary nav-ish pages only
    nav_links = [l for l in nav_links if l in {
        '/dashboard','/sessions','/sessions/new','/ready','/plan/wizard','/plan/current','/recovery','/analytics','/assistant','/media','/content-packs','/library','/avatars','/avatar-3d','/settings/profile','/exports','/restore'
    }]
    assert len(nav_links) == len(set(nav_links))
    for route in sorted(set(nav_links)):
        res = client.get(route)
        assert res.status_code == 200, route


def test_release_zip_creation_script(tmp_path, monkeypatch):
    import subprocess, zipfile
    from pathlib import Path
    monkeypatch.chdir(tmp_path)
    repo_root = Path(__file__).resolve().parent
    out = subprocess.run(['python', str(repo_root / 'tools' / 'make_release.py')], cwd=repo_root, capture_output=True, text=True)
    assert out.returncode == 0, out.stderr
    releases = sorted((repo_root / 'RELEASES').glob('flowform-release-*.zip'))
    assert releases
    zf = zipfile.ZipFile(releases[-1])
    names = zf.namelist()
    assert 'VERSION_STAMP.txt' in names
